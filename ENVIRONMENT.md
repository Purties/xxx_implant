...
> 更新：2026-09-15（r6.7 执行次序重排：存活定性→M1→反检测；126 推广降级为按需扩展）
...
### P0 —— 阶段 2 功能复刻（当前主线，r6.7 重排）

1. **--spawn 存活 30s 问题定性**（最高优先，用户配合：启动一次游戏）
   - 现象：09-15 11:17 --spawn 注入的进程在解析完成后 ~30s 自行退出（无 WER/转储）；此前 race/启动器注入无此现象
   - 对照实验：同构建下 race 注入（启动器启动）与 --spawn 注入的存活时长对比；观察退出时点与注入方式/模块加载时机的相关性
   - 若证实为反作弊检测行为 → 反检测工程化（P1-4/5/6）升级为阻断项
2. **端到端功能验证（M1 无 UI 最小可用）**（用户配合：新版进游戏世界，小号）
   ...
   - 5 钩已全部解析（定位阻塞清零），只差功能逻辑
3. **旧版行为观察**（用户方便时，与 1/2 并行）：按 UNPACKING_REPORT §5.10.4 协议在旧客户端逐项开/关功能并回报现象 + 页签截图 → 补全行为规格（M3 输入）

> ⚠ **已降级（r6.7）**："新客户端全钩子解析/推广到全 126"不再是首要任务——21 条三构建唯一签名已覆盖 23/126（§5.9.5e），剩余 103 个离线不可达（无真值对），改为**按需扩展**：实现某功能需要哪个钩子再现场解析哪个。
...
12. 交接文档每里程碑更新（当前 r6.7）；本文档随环境变化更新
